...
namespace Yamed.Control.Editors
{
    ...
    public class FiasHouse2
    {
        public Guid HOUSEGUID { get; set; }
        public string NAME { get; set; }
    }

    public class FiasAddress2
    {
    ...
    /// </summary>
    public partial class FiasControl2 : LayoutGroup
    {
        
        public bool from_fias = false;
        private readonly string _connectionString; 
        public FiasAddress2 Address;
        public string btn_1;
        public FiasControl2()
        {
            InitializeComponent();
            ...
            List<FiasStreet2> regionList = new List<FiasStreet2>();
            ...
            var mekTask = Task.Factory.StartNew(() =>
            {
                try
                {
                    regionList = Reader2List.CustomSelect<FiasStreet2>(@"
SELECT AOGUID ID, FORMALNAME + ' - ' + aot.SOCRNAME NAME, AOLEVEL [LEVEL]
  ...
  order by formalname", _connectionString);
        ...
        {
            var rnList =
                Reader2List.CustomSelect<FiasStreet2>(
                    $@"
            SELECT AOGUID ID, FORMALNAME + ' - ' + aot.SOCRNAME NAME, AOLEVEL[LEVEL]
 FROM [dbo].[AddressObjects] ao
...
where PARENTGUID = '{
  ...
  order by formalname", _connectionString);
        ...
        {
            var ulList =Reader2List.CustomSelect<FiasStreet2>(
                    $@"
            SELECT AOGUID ID, FORMALNAME + ' - ' + aot.SOCRNAME NAME, AOLEVEL[LEVEL]
 FROM [dbo].[AddressObjects] ao
join [dbo].[AddressObjectTypes]
...
where PARENTGUID = '{
  ...
  order by formalname", _connectionString);
        ...
        {
            var houseList =
                Reader2List.CustomSelect<FiasHouse2>(
                    $@"
DECLARE @tt TABLE
...
SELECT HOUSEGUID, ISNULL('д. ' + housenum, '') + ISNULL('  к. ''' + BUILDNUM + '''', '') + ISNULL('  стр. ''' + STRUCNUM  + '''', '')
FROM [dbo].[Houses]
WHERE aoguid='{id}'
...
GROUP BY NAME,HOUSEGUID", _connectionString);
        ...
        //private void reg_np_OnEditValueChanged(object sender, EditValueChangedEventArgs e)
        //{
        //    //var value = ((BaseEdit)sender).EditValue;
        ...
        //private void reg_ul_OnEditValueChanged(object sender, EditValueChangedEventArgs e)
        //{
        //    //var value = ((BaseEdit)sender).EditValue;
        ...
        private void bomj_PreviewKeyDown(object sender, System.Windows.Input.KeyEventArgs e)
        {
            ...
            if (e.Key == Key.Enter)
            {
                ...
                //WpfApp2.Window2 w2 = new WpfApp2.Window2(iid, btn, ppol, cel_viz, sppz, dobr, petit, prz_cod);
                //w2.tabs.SelectedIndex = 2;
                //ttab = true;
                //if (Method != null)
                //{
                //    Method(ttab);
        ...
        {
            //var dom = reg_dom.EditValue;
            //MessageBox.Show($"DisplayMember: {reg_dom.DisplayMember}, edit value:{reg_dom.EditValue.ToString()}");
        }
    }
}
